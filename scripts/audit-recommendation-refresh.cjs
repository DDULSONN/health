...
(async () => {
  ...
  const samples = [...grouped.values()].filter(rows => rows.length >= 2).slice(0, Number(process.env.AUDIT_LIMIT || 20));
  ...
  for (const rows of samples) {
    ...
    console.log(JSON.stringify({ name: nameRes.data?.name, card: source.id.slice(0,8), sex:source.sex, pool:pool.length, overlap, improvedOverlap, shown:after.length, hoursBetween:Math.round((Date.parse(rows[0].refreshed_at)-Date.parse(rows[1].refreshed_at))/3600000), latest:rows[0].refreshed_at }));
  }
