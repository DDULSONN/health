...
(async () => {
  ...
  assert.equal(await city.hasCityViewCardAccess(dbFor(() => ({ data: [{ ...grantRow, note: "paid" }], error: null })), "viewer", ids[10], "서울"), true);
  ...
  grant = { requestId: "grant", preview: true, snapshotCardIds: ids.slice(0, 10), accessExpiresAt: "2099-01-01" };
