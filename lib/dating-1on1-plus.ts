...
export async function grantOneOnOnePlus(
  ...
  options: {
    ...
    grantKey: string;
    durationDays?: number;
    requireActiveCard?: boolean;
  }
) {
  if (options.requireActiveCard !== false) {
    const activeCardResult = await admin
    ...
    if (!activeCardResult.data) {
      throw new Error("활성 1:1 신청서가 있어야 플러스를 이용할 수 있습니다.");
    }
  }
