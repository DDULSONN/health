...
const ROOT_PREFIX = "admin-application-backups";
...
export async function GET(request: Request) {
  ...
  try {
    await collectExpiredPaths(admin, ROOT_PREFIX, Date.now() - RETENTION_MS, expiredPaths);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.error("[cron dating-apply-photo-backups-cleanup] list failed", message);
  ...
  if (removeRes.error) {
    console.error("[cron dating-apply-photo-backups-cleanup] remove failed", removeRes.error.message);
