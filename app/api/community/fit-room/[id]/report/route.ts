...
export async function POST(request: Request, { params }: RouteCtx) {
  ...
  const rateLimit = await checkRouteRateLimit({
    ...
    userLimitPerMin: 8,
    ipLimitPerMin: 40,
    path: "/api/community/fit-room/report",
  });
  ...
  if (entryError || !entry || entry.deleted_at) {
    return NextResponse.json({ error: "신고할 인증 사진을 찾지 못했습니다." }, { status: 404 });
  }

  if (entry.user_id === user.id) {
    return NextResponse.json({ error: "내가 올린 사진은 신고할 수 없습니다." }, { status: 400 });
  }
  ...
  if (error) {
    ...
    console.error("[POST /api/community/fit-room/[id]/report] failed", error);
