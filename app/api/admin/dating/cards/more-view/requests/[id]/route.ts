...
import { approveMoreViewRequest, rejectMoreViewRequest } from "@/lib/dating-purchase-fulfillment";
  ...
  try {
    ...
    const item =
      status === "approved"
        ...
        : await rejectMoreViewRequest(admin, {
            requestId: id,
            reviewedByUserId: user.id,
            note,
          });
    ...
    if (!item) {
      return NextResponse.json({ ok: false, code: "NOT_PENDING", requestId, message: "대기중 요청만 처리할 수 있습니다." }, { status: 409 });
    }
