...
function mapDbErrorToHttp(code?: string | null): { status: number; apiCode: ApiCode; message: string } {
  if (code === "23505") return { status: 409, apiCode: "DUPLICATE_APPLICATION", message: "이미 해당 카드에 지원했습니다." };
  if (code === "23502") return { status: 400, apiCode: "VALIDATION_ERROR", message: "필수 입력값이 누락되었습니다." };
  if (code === "23503") return { status: 400, apiCode: "VALIDATION_ERROR", message: "카드 또는 회원 정보가 올바르지 않습니다." };
  if (code === "42501") return { status: 403, apiCode: "FORBIDDEN", message: "지원 처리 권한이 없습니다." };
  if (code === "PGRST202" || code === "PGRST204" || code === "42883") {
    return { status: 503, apiCode: "SCHEMA_MISMATCH", message: "지원 처리에 필요한 서버 설정이 아직 적용되지 않았습니다. 잠시 후 다시 시도해주세요." };
  }
  return { status: 500, apiCode: "DATABASE_ERROR", message: "지원 처리 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요." };
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!applicantDisplayNickname) {
      return jsonResponse(400, "NICKNAME_REQUIRED", requestId, "닉네임 설정 후 이용할 수 있습니다.", {
    ...
    if (card.status === "expired") {
      return jsonResponse(410, "CARD_EXPIRED", requestId, "만료된 카드에는 지원할 수 없습니다.");
    }
    ...
    if (card.status !== "public" && !allowedByMoreView && !allowedByCityView) {
      return jsonResponse(403, "FORBIDDEN", requestId, "현재 공개되거나 열람 가능한 카드가 아닙니다.");
    }
    if (card.status === "public" && (!card.expires_at || new Date(card.expires_at).getTime() <= Date.now())) {
      return jsonResponse(410, "CARD_EXPIRED", requestId, "공개 기간이 끝난 카드입니다.");
    }
    ...
    if (!insertRes.data?.id) {
      ...
      return jsonResponse(500, "DATABASE_ERROR", requestId, "지원 저장 결과를 확인하지 못했습니다. 잠시 후 다시 시도해주세요.");
    }
    ...
    return jsonResponse(500, "INTERNAL_SERVER_ERROR", requestId, "지원 처리 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.", undefined, dbError);
  }
}
