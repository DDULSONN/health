...
const OneOnOneHomePanel = dynamic(() => import("@/components/dating/OneOnOneHomePanel"), {
  ssr: false,
  loading: () => (
    <section className="mb-5 rounded-2xl border border-rose-100 bg-white p-5 text-sm text-neutral-500 shadow-sm">
      1대1 매칭 정보를 불러오고 있어요...
    </section>
  ),
});
...
export default function OpenCardsPage() {
  ...
  const [homeFeatureTabReady, setHomeFeatureTabReady] = useState(false);
  ...
  useEffect(() => {
    if (!homeFeatureTabReady || homeFeatureTab !== "open_cards") return;
    let cancelled = false;
  ...
  }, [homeFeatureTab, homeFeatureTabReady]);
  ...
  useEffect(() => {
    queueMicrotask(async () => {
      try {
        ...
        setViewerPhoneVerified(bootstrap.profile.profile?.phone_verified === true);
        if (bootstrap.audience) setCardsAudience(bootstrap.audience);
        setMyOpenCards(Array.isArray(bootstrap.openCards.items) ? bootstrap.openCards.items as unknown as MyOpenCard[] : []);
        setHasActiveOneOnOneProfile(Boolean(bootstrap.oneOnOne.activeRequestStatus));
        setHomeProfilePresenceReady(true);
      } catch (error) {
        if ((error as Error & { status?: number }).status === 401) {
          ...
          setCardsAudience({
            status: "guest",
            viewerSex: null,
            targetSex: activeSexRef.current,
            source: null,
            canSwitchSex: true,
  ...
  const reloadMyOpenCards = useCallback(async () => {
    ...
    setHomeProfilePresenceReady(false);
    try {
      const bootstrap = await loadDatingProfileBootstrap({ force: true });
      setMyOpenCards(Array.isArray(bootstrap.openCards.items) ? bootstrap.openCards.items as unknown as MyOpenCard[] : []);
      setHasActiveOneOnOneProfile(Boolean(bootstrap.oneOnOne.activeRequestStatus));
      setHomeProfilePresenceReady(true);
    } catch {
  ...
  useEffect(() => {
    if (!homeFeatureTabReady || homeFeatureTab !== "quick_match" || !viewerLoggedIn) {
  ...
  }, [homeFeatureTab, homeFeatureTabReady, viewerLoggedIn]);
  ...
  const reloadOneOnOneHome = useCallback(async () => {
    ...
    setOneOnOneHomeError("");
    try {
      const [bootstrap, myCardsRes, matchesRes, recommendationsRes] = await Promise.all([
        loadDatingProfileBootstrap(),
      ...
      const [myCardsBody, matchesBody, recommendationsBody] = await Promise.all([
  ...
  useEffect(() => {
    if (!homeFeatureTabReady || homeFeatureTab !== "one_on_one") return;
    let cancelled = false;
  ...
  }, [homeFeatureTab, homeFeatureTabReady, reloadOneOnOneHome]);
  ...
  const handleOpenCardSexChange = useCallback(
    async (nextSex: DatingSex) => {
      ...
      if (homeFeatureTab !== "open_cards") return;
    ...
    [cardsAudience?.canSwitchSex, homeFeatureTab, loadInitial, refreshSecondary]
  );
  ...
  useEffect(() => {
    if (!snapshotReady || !homeFeatureTabReady || homeFeatureTab !== "open_cards") return;
    queueMicrotask(() => {
      void loadInitial({ silent: Boolean(restoredSnapshot) });
    });
  }, [homeFeatureTab, homeFeatureTabReady, snapshotReady, restoredSnapshot, loadInitial]);
  ...
  }, [cardsAudience, homeFeatureTab, homeFeatureTabReady]);
  ...
  useEffect(() => {
    setSwipeMessage("");
    if (!homeFeatureTabReady || homeFeatureTab !== "quick_match") return;
  ...
  }, [activeSex, cardsAudience, homeFeatureTab, homeFeatureTabReady, loadSwipe]);
...
function PaidCardRow({ card, viewerLoggedIn }: { card: PaidCard; viewerLoggedIn: boolean }) {
  ...
  const warmRoute = useCallback(() => {
    cachePaidCardDetail(card.id, {
      ...
      photo_visibility: card.photo_visibility,
    });
